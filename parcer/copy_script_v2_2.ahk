...
; "Константа" — число: любое количество цифр подряд (без верхнего предела!),
; опционально + группы по 3 цифры через пробел (тысячи: "16 800"),
; опционально дробная часть через запятую/точку.
; ВАЖНО: первый блок именно "\d+", а не "\d{1,3}" — иначе числа от 1000
; и выше БЕЗ пробела-разделителя (например "1118", "27950") обрезаются
; до 3 цифр движком регэкспа (баг, найденный на реальных данных в v2.1).
